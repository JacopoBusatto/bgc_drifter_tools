...
def bbp_csv_to_dataframe(path: str | Path, *, platform_id: str | None = None) -> pd.DataFrame:
    ...
    Read one bbp raw CSV (e.g. 300534065378180.csv) and return canonical dataframe:
      platform_id, time_utc, lat, lon, bbp_470_m1, bbp_532_m1, [temp_ctd_c], [chl]
    ...
    # rename known columns (only those present)
    ...
    # numeric coercion (core + optional)
    num_cols = ["lat", "lon", "bbp_470_m1", "bbp_532_m1"]
    for c in ["temp_ctd_c", "chl"]:
        if c in df.columns:
            num_cols.append(c)
