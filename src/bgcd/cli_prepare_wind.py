...
from .io import canonicalize_wind_df


def _parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="bgcd.cli_prepare_wind",
        description="Split and normalize raw wind_data.csv into per-platform canonical files.",
    )
    p.add_argument("--input-file", required=True, help="Path to raw wind_data.csv")
...
def main(argv=None) -> int:
    ...
    for pid, raw_df in res.chunks.items():
        try:
            canon = canonicalize_wind_df(raw_df)
...
python -m bgcd.cli_prepare_wind `
  --input-file ""C:/Users/Jacopo/OneDrive - CNR/BGC-SVP/DATI_PLATFORMS/wind_data.csv" `
  --output-dir ""C:/Users/Jacopo/OneDrive - CNR/BGC-SVP/DATI_PLATFORMS/db_wind_new"
"""